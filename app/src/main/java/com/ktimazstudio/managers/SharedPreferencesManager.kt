...
class SharedPreferencesManager(context: Context) {
    ...
    // Enhanced security methods with simple encryption
    ...
    // FIXED: Complete implementation of importSettings method
    fun importSettings(settings: Map<String, Any?>) {
        try {
            ...
            // Log error and continue - don't crash on import failure
            e.printStackTrace()
        }
    }
    ...
    fun clearUserData() {
        ...
        securePrefs.edit().apply {
            ...
            remove(SECURE_KEY_BIOMETRIC_ENABLED)
            apply()
        }
    }
    ...
    fun exportSettings(): Map<String, Any?> {
        return prefs.all.filterKeys { key ->
            !key.contains("login") && !key.contains("username") && !key.contains("token")
        }
    }
    ...
    fun createSettingsBackup(): String {
        ...
        val json = settings.entries.joinToString(",") { (key, value) ->
            "\"$key\":\"$value\""
        }
        return "{$json}"
    }

    fun restoreFromBackup(backupString: String): Boolean {
        return try {
            // Simple JSON parsing for backup restoration
            val cleanJson = backupString.trim('{', '}')
    ...
    fun getPerformanceMetrics(): Map<String, Long> {
        return mapOf(
            "settings_count" to prefs.all.size.toLong(),
            "secure_settings_count" to securePrefs.all.size.toLong(),
            "last_access_time" to System.currentTimeMillis()
        )
    }
    ...
    fun migrateSettings(fromVersion: Int, toVersion: Int) {
        when {
    ...
    private fun migrateToV3() {
        ...
        if (!prefs.contains(KEY_SECURE_MODE)) {
